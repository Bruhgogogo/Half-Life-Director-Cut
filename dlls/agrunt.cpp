...
enum
{
	...
	SCHED_AGRUNT_FOLLOW,      // New: follow player
	SCHED_AGRUNT_FACE_TARGET, // New: face target (player)
};
...
// Follow range for friendly alien grunt
static constexpr float AGRUNT_FOLLOW_RANGE = 128.0f;
static constexpr float AGRUNT_MELEE_DIST = 100.0f;
...
public:
	...
	BOOL	m_bFriendly;			// If TRUE, alien grunt is friendly to player and player allies
...
TYPEDESCRIPTION	CAGrunt::m_SaveData[] =
{
	...
	DEFINE_FIELD(CAGrunt, m_bFriendly, FIELD_BOOLEAN),
	DEFINE_FIELD(CAGrunt, m_flPlayerDamage, FIELD_FLOAT),
};
...
//=========================================================
// ObjectCaps - Allow player to use friendly alien grunt
//=========================================================
int CAGrunt::ObjectCaps(void)
{
	if (m_bFriendly)
	{
...
//=========================================================
void CAGrunt::KeyValue(KeyValueData* pkvd)
{
...
{
	// Friendly alien grunts are allied with player and player allies
	if (m_bFriendly)
	{
		if (pTarget->IsPlayer())
			return R_AL;

		// All player allies are friendly
		if (pTarget->Classify() == CLASS_PLAYER_ALLY)
			return R_AL;

		// Don't attack other friendly alien grunts (they are also CLASS_PLAYER_ALLY)
		if (FClassnameIs(pTarget->pev, "monster_alien_grunt"))
		{
			CAGrunt* pGrunt = static_cast<CAGrunt*>(pTarget);
			if (pGrunt && pGrunt->m_bFriendly)
				return R_AL;
		}

		// Don't attack friendly Barney
...
void CAGrunt::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case AGRUNT_AE_HORNET5:
	{
		...
		{
		case 0:	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "agrunt/ag_fire1.wav", 1.0f, ATTN_NORM, 0, 100);	break;
		case 1:	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "agrunt/ag_fire2.wav", 1.0f, ATTN_NORM, 0, 100);	break;
		case 2:	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "agrunt/ag_fire3.wav", 1.0f, ATTN_NORM, 0, 100);	break;
		}
	...
	case AGRUNT_AE_LEFT_FOOT:
		...
		{
			// left foot
		case 0:	EMIT_SOUND_DYN(ENT(pev), CHAN_BODY, "player/pl_ladder2.wav", 1.0f, ATTN_NORM, 0, 70);	break;
		case 1:	EMIT_SOUND_DYN(ENT(pev), CHAN_BODY, "player/pl_ladder4.wav", 1.0f, ATTN_NORM, 0, 70);	break;
		}
		break;
	case AGRUNT_AE_RIGHT_FOOT:
		...
		switch (RANDOM_LONG(0, 1))
		{
		case 0:	EMIT_SOUND_DYN(ENT(pev), CHAN_BODY, "player/pl_ladder1.wav", 1.0f, ATTN_NORM, 0, 70);	break;
		case 1:	EMIT_SOUND_DYN(ENT(pev), CHAN_BODY, "player/pl_ladder3.wav", 1.0f, ATTN_NORM, 0, 70);	break;
		}
		break;
...
void CAGrunt::Spawn()
{
	...
	// Only allow following if alien grunt is friendly
	if (m_bFriendly)
	{
		SetUse(&CAGrunt::FollowerUse);
	}
	else
	{
		SetUse(nullptr);
...
void CAGrunt::Precache()
{
	...
	for (i = 0; i < ARRAYSIZE(pAttackHitSounds); i++)
		PRECACHE_SOUND(const_cast<char*>(pAttackHitSounds[i]));

	for (i = 0; i < ARRAYSIZE(pAttackMissSounds); i++)
		PRECACHE_SOUND(const_cast<char*>(pAttackMissSounds[i]));

	for (i = 0; i < ARRAYSIZE(pIdleSounds); i++)
		PRECACHE_SOUND(const_cast<char*>(pIdleSounds[i]));

	for (i = 0; i < ARRAYSIZE(pDieSounds); i++)
		PRECACHE_SOUND(const_cast<char*>(pDieSounds[i]));

	for (i = 0; i < ARRAYSIZE(pPainSounds); i++)
		PRECACHE_SOUND(const_cast<char*>(pPainSounds[i]));

	for (i = 0; i < ARRAYSIZE(pAttackSounds); i++)
		PRECACHE_SOUND(const_cast<char*>(pAttackSounds[i]));

	for (i = 0; i < ARRAYSIZE(pAlertSounds); i++)
		PRECACHE_SOUND(const_cast<char*>(pAlertSounds[i]));
...
Task_t	tlAGruntHiddenRangeAttack1[] =
{
	...
	{ TASK_AGRUNT_SETUP_HIDE_ATTACK,	0.0f				},
	{ TASK_STOP_MOVING,					0.0f				},
	{ TASK_FACE_IDEAL,					0.0f				},
	{ TASK_RANGE_ATTACK1_NOTURN,		0.0f				},
};
...
//=========================================================
// Follow schedule - Alien Grunt follows the player
//=========================================================
Task_t tlAGruntFollow[] =
{
	{ TASK_MOVE_TO_TARGET_RANGE, AGRUNT_FOLLOW_RANGE },
	{ TASK_SET_SCHEDULE, static_cast<float>(SCHED_AGRUNT_FACE_TARGET) },
};

Schedule_t slAGruntFollow[] =
{
	{
		tlAGruntFollow,
		ARRAYSIZE(tlAGruntFollow),
		bits_COND_NEW_ENEMY |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_HEAR_SOUND |
...
//=========================================================
// Face target schedule - Alien Grunt faces the player
//=========================================================
Task_t tlAGruntFaceTarget[] =
{
	{ TASK_SET_ACTIVITY, static_cast<float>(ACT_IDLE) },
	{ TASK_FACE_TARGET, 0.0f },
	{ TASK_SET_ACTIVITY, static_cast<float>(ACT_IDLE) },
	{ TASK_SET_SCHEDULE, static_cast<float>(SCHED_AGRUNT_FOLLOW) },
};

Schedule_t slAGruntFaceTarget[] =
{
	{
		tlAGruntFaceTarget,
		ARRAYSIZE(tlAGruntFaceTarget),
		bits_COND_NEW_ENEMY |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_HEAR_SOUND |
...
{
	// Friendly alien grunts won't attack player or player allies
...
//=========================================================
// TakeDamage - If friendly and attacked by player, become hostile
//=========================================================
...
void CAGrunt::StartTask(Task_t* pTask)
{
		...
		// ideally, this spot is along a line that is perpendicular to a line drawn from the agrunt to the enemy.
	{
		...
		CBaseMonster* pEnemyMonsterPtr = m_hEnemy->MyMonsterPointer();
...
{
	// If following, prioritize follow/face schedules
	if (IsFollowing() && m_hTargetEnt != NULL)
	{
		...
		else
		{
			...
			{
				return GetScheduleOfType(SCHED_AGRUNT_FACE_TARGET);
			}
		}
	}
	...
	{
		// If following, don't let other things distract
		if (IsFollowing())
		{
			return GetScheduleOfType(SCHED_AGRUNT_FACE_TARGET);
		}
...
Schedule_t* CAGrunt::GetScheduleOfType(int Type)
{
	...
	case SCHED_AGRUNT_FOLLOW:
		return &slAGruntFollow[0];

	case SCHED_AGRUNT_FACE_TARGET:
		return &slAGruntFaceTarget[0];
...
//=========================================================
// FollowerUse - Allow player to command friendly alien grunt
//=========================================================
void CAGrunt::FollowerUse(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	// Only players can use friendly alien grunts
	if (!pActivator || !pActivator->IsPlayer())
		return;

	// Only friendly alien grunts can be used
	if (!m_bFriendly)
		return;
...
//=========================================================
void CAGrunt::DeclineFollowing(void)
{
...
//=========================================================
void CAGrunt::StartFollowing(CBaseEntity* pLeader)
{
	if (pLeader == NULL)
		return;
	...
	ChangeSchedule(GetScheduleOfType(SCHED_AGRUNT_FOLLOW));
}

//=========================================================
// IsFollowing - Check if currently following a leader
//=========================================================
BOOL CAGrunt::IsFollowing(void)
{
...
//=========================================================
void CAGrunt::StopFollowing(BOOL bClearSchedule)
{
	// Clear the target
	m_hTargetEnt = NULL;
