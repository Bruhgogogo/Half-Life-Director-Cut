...
// Follow range for friendly bullsquid
static constexpr float BULLSQUID_FOLLOW_RANGE = 128.0f;
// Sprint distance threshold
static constexpr float SQUID_SPRINT_DIST = 256.0f;
// Spit sprite index
static int g_iSquidSpitSprite = 0;

//=========================================================
// monster-specific schedule types
//=========================================================
enum class SquidSchedule : int
{
	HURTHOP = LAST_COMMON_SCHEDULE + 1,
	SMELLFOOD,
	SEECRAB,
	EAT,
	SNIFF_AND_EAT,
	...
	FACE_TARGET, // New: face target (player)
};
...
void CSquidSpit::Touch(CBaseEntity* pOther)
{
	...
	{
	case 0:
		EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "bullchicken/bc_spithit1.wav", 1.0f, ATTN_NORM, 0, iPitch);
		break;
	case 1:
		EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "bullchicken/bc_spithit2.wav", 1.0f, ATTN_NORM, 0, iPitch);
		break;
	}
	...
	if (!pOther->pev->takedamage)
	{
		...
		WRITE_SHORT(g_iSquidSpitSprite);	// model
		WRITE_BYTE(5);			// count
		WRITE_BYTE(30);			// speed
...
//=========================================================
static constexpr int BSQUID_AE_SPIT = 1;
static constexpr int BSQUID_AE_BITE = 2;
static constexpr int BSQUID_AE_BLINK = 3;
static constexpr int BSQUID_AE_TAILWHIP = 4;
static constexpr int BSQUID_AE_HOP = 5;
static constexpr int BSQUID_AE_THROW = 6;
...
public:
	...
	BOOL m_bFriendly;			// If TRUE, bullsquid is friendly to player and player allies
...
TYPEDESCRIPTION	CBullsquid::m_SaveData[] =
{
	...
	DEFINE_FIELD(CBullsquid, m_bFriendly, FIELD_BOOLEAN),
	DEFINE_FIELD(CBullsquid, m_flPlayerDamage, FIELD_FLOAT),
};

IMPLEMENT_SAVERESTORE(CBullsquid, CBaseMonster);

//=========================================================
// ObjectCaps - Allow player to use friendly bullsquid
//=========================================================
int CBullsquid::ObjectCaps(void)
{
	if (m_bFriendly)
	{
...
//=========================================================
// KeyValue - Handle custom keyvalues
//=========================================================
void CBullsquid::KeyValue(KeyValueData* pkvd)
{
...
int CBullsquid::IgnoreConditions(void)
{
	...
	// If following, ignore food scents (follow priority over eating)
	if (IsFollowing())
	{
...
{
	// Friendly bullsquid are allied with player and player allies
	if (m_bFriendly)
	{
		if (pTarget->IsPlayer())
			return R_AL;
		...
		// Don't attack other friendly bullsquids
		if (FClassnameIs(pTarget->pev, "monster_bullchicken"))
		{
			CBullsquid* pSquid = static_cast<CBullsquid*>(pTarget);
			if (pSquid && pSquid->m_bFriendly)
				return R_AL;
		}
...
int CBullsquid::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	...
	// it will swerve. (whew).
	if (m_hEnemy != nullptr && IsMoving() && pevAttacker == m_hEnemy->pev && gpGlobals->time - m_flLastHurtTime > 3.0f)
	{
...
{
	// Friendly bullsquid won't attack player or player allies
	...
	if (flDist > 64.0f && flDist <= 784.0f && flDot >= 0.5f && gpGlobals->time >= m_flNextSpitTime)
	{
		if (m_hEnemy != nullptr)
		{
			if (fabsf(pev->origin.z - m_hEnemy->pev->origin.z) > 256.0f)
			{
...
BOOL CBullsquid::CheckMeleeAttack1(float flDot, float flDist)
{
	...
	if (m_hEnemy && m_hEnemy->pev->health <= gSkillData.bullsquidDmgWhip && flDist <= 85.0f && flDot >= 0.7f)
	{
		return TRUE;
	}
	return FALSE;
...
BOOL CBullsquid::CheckMeleeAttack2(float flDot, float flDist)
{
	...
	if (flDist <= 85.0f && flDot >= 0.7f && !HasConditions(bits_COND_CAN_MELEE_ATTACK1))
	{
		return TRUE;
	}
	return FALSE;
...
{
	static const short sSquidHints[] =
	{
...
// IdleSound 
//=========================================================
static constexpr float SQUID_ATTN_IDLE = 1.5f;
...
void CBullsquid::IdleSound(void)
{
	...
	{
	case 0:
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "bullchicken/bc_idle1.wav", 1.0f, SQUID_ATTN_IDLE);
		break;
	case 1:
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "bullchicken/bc_idle2.wav", 1.0f, SQUID_ATTN_IDLE);
		break;
	case 2:
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "bullchicken/bc_idle3.wav", 1.0f, SQUID_ATTN_IDLE);
		break;
	case 3:
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "bullchicken/bc_idle4.wav", 1.0f, SQUID_ATTN_IDLE);
		break;
	case 4:
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "bullchicken/bc_idle5.wav", 1.0f, SQUID_ATTN_IDLE);
		break;
	}
}
...
void CBullsquid::PainSound(void)
{
	...
	{
	case 0:
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "bullchicken/bc_pain1.wav", 1.0f, ATTN_NORM, 0, iPitch);
		break;
	case 1:
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "bullchicken/bc_pain2.wav", 1.0f, ATTN_NORM, 0, iPitch);
		break;
	case 2:
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "bullchicken/bc_pain3.wav", 1.0f, ATTN_NORM, 0, iPitch);
		break;
	case 3:
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "bullchicken/bc_pain4.wav", 1.0f, ATTN_NORM, 0, iPitch);
		break;
	}
}
...
void CBullsquid::AlertSound(void)
{
	...
	{
	case 0:
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "bullchicken/bc_idle1.wav", 1.0f, ATTN_NORM, 0, iPitch);
		break;
	case 1:
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "bullchicken/bc_idle2.wav", 1.0f, ATTN_NORM, 0, iPitch);
		break;
	}
}
...
void CBullsquid::DeathSound(void)
{
	...
	{
	case 0:
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "bullchicken/bc_die1.wav", 1.0f, ATTN_NORM);
		break;
	case 1:
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "bullchicken/bc_die2.wav", 1.0f, ATTN_NORM);
		break;
	case 2:
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "bullchicken/bc_die3.wav", 1.0f, ATTN_NORM);
		break;
	}
}
...
void CBullsquid::AttackSound(void)
{
	...
	{
	case 0:
		EMIT_SOUND(ENT(pev), CHAN_WEAPON, "bullchicken/bc_attack2.wav", 1.0f, ATTN_NORM);
		break;
	case 1:
		EMIT_SOUND(ENT(pev), CHAN_WEAPON, "bullchicken/bc_attack3.wav", 1.0f, ATTN_NORM);
		break;
	}
}
...
void CBullsquid::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	{
		// SOUND HERE!
		CBaseEntity* pHurt = CheckTraceHullAttack(70.0f, gSkillData.bullsquidDmgBite, DMG_SLASH);
	...
	{
		CBaseEntity* pHurt = CheckTraceHullAttack(70.0f, gSkillData.bullsquidDmgWhip, DMG_CLUB | DMG_ALWAYSGIB);
		if (pHurt)
		{
	...
	case BSQUID_AE_THROW:
	{
		...
		CBaseEntity* pHurt = CheckTraceHullAttack(70.0f, 0.0f, 0);
...
void CBullsquid::Spawn()
{
	...
	// Only allow following if bullsquid is friendly
	if (m_bFriendly)
	{
		SetUse(&CBullsquid::FollowerUse);
	}
	else
	{
		SetUse(nullptr);
...
void CBullsquid::Precache()
{
	...
	g_iSquidSpitSprite = PRECACHE_MODEL("sprites/tinyspit.spr");	// client side spittle.
...
//=========================================================
// Follow schedule - Bullsquid follows the player
//=========================================================
Task_t tlSquidFollow[] =
{
	{ TASK_MOVE_TO_TARGET_RANGE, BULLSQUID_FOLLOW_RANGE },
	{ TASK_SET_SCHEDULE, static_cast<float>(SCHED_TARGET_FACE) },
};

Schedule_t slSquidFollow[] =
{
	{
		tlSquidFollow,
		ARRAYSIZE(tlSquidFollow),
		bits_COND_NEW_ENEMY |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_HEAR_SOUND |
...
//=========================================================
// Face target schedule - Bullsquid faces the player
//=========================================================
Task_t tlSquidFaceTarget[] =
{
	{ TASK_SET_ACTIVITY, static_cast<float>(ACT_IDLE) },
...
Schedule_t slSquidFaceTarget[] =
{
	{
		tlSquidFaceTarget,
		ARRAYSIZE(tlSquidFaceTarget),
		bits_COND_NEW_ENEMY |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_HEAR_SOUND |
...
{
	// If following, prioritize follow/face schedules over eating
	if (IsFollowing() && m_hTargetEnt != nullptr)
	{
		if (!m_hTargetEnt->IsAlive())
		{
			StopFollowing(FALSE);
			// Fall through to normal schedule selection
		}
		else
		{
			// Check if we need to handle enemy threats first (combat overrides follow)
			if (m_MonsterState == MONSTERSTATE_COMBAT && m_hEnemy != nullptr)
			{
				// Combat takes priority - fall through to combat handling
			}
			else
			{
				return GetScheduleOfType(static_cast<int>(SquidSchedule::FACE_TARGET));
			}
		}
	}
	...
	{
		// If following, don't let eating distract from following
		if (IsFollowing())
		{
			return GetScheduleOfType(static_cast<int>(SquidSchedule::FACE_TARGET));
		}
		...
		{
			return GetScheduleOfType(static_cast<int>(SquidSchedule::HURTHOP));
		}
		...
		if (HasConditions(bits_COND_SMELL_FOOD))
		{
			...
			if (pSound && (!FInViewCone(&pSound->m_vecOrigin) || !FVisible(pSound->m_vecOrigin)))
			{
				...
				return GetScheduleOfType(static_cast<int>(SquidSchedule::SNIFF_AND_EAT));
			}

			// food is right out in the open. Just go get it.
			return GetScheduleOfType(static_cast<int>(SquidSchedule::EAT));
		}
		...
		if (HasConditions(bits_COND_SMELL))
		{
			...
			CSound* pSound = PBestScent();
			if (pSound)
				return GetScheduleOfType(static_cast<int>(SquidSchedule::WALLOW));
		}
	...
	case MONSTERSTATE_COMBAT:
	{
		...
		if (m_bFriendly && m_hEnemy != nullptr)
		{
			if (m_hEnemy->IsPlayer() || m_hEnemy->Classify() == CLASS_PLAYER_ALLY)
			{
				m_hEnemy = nullptr;
		...
		if (HasConditions(bits_COND_NEW_ENEMY))
		{
			if (m_fCanThreatDisplay && IRelationship(m_hEnemy) == R_HT)
			{
				...
				return GetScheduleOfType(static_cast<int>(SquidSchedule::SEECRAB));
			}
			else
			{
		...
		// If following, eating is lower priority than combat
		if (!IsFollowing() && HasConditions(bits_COND_SMELL_FOOD))
		{
			CSound* pSound = PBestScent();
	...
	{
		// If following, return follow schedule even in IDLE state
		if (IsFollowing() && m_hTargetEnt != nullptr)
		{
			if (!m_hTargetEnt->IsAlive())
			{
...
Schedule_t* CBullsquid::GetScheduleOfType(int Type)
{
	...
	case static_cast<int>(SquidSchedule::HURTHOP):
		return &slSquidHurtHop[0];
	case static_cast<int>(SquidSchedule::SEECRAB):
		return &slSquidSeeCrab[0];
	case static_cast<int>(SquidSchedule::EAT):
		return &slSquidEat[0];
	case static_cast<int>(SquidSchedule::SNIFF_AND_EAT):
		return &slSquidSniffAndEat[0];
	case static_cast<int>(SquidSchedule::WALLOW):
		return &slSquidWallow[0];
	case SCHED_CHASE_ENEMY:
		return &slSquidChaseEnemy[0];
	case static_cast<int>(SquidSchedule::FOLLOW):
		return &slSquidFollow[0];
	case static_cast<int>(SquidSchedule::FACE_TARGET):
		return &slSquidFaceTarget[0];
...
void CBullsquid::StartTask(Task_t* pTask)
{
	...
	case TASK_MELEE_ATTACK2:
	{
		...
		{
		case 0:
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "bullchicken/bc_attackgrowl.wav", 1.0f, ATTN_NORM);
			break;
		case 1:
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "bullchicken/bc_attackgrowl2.wav", 1.0f, ATTN_NORM);
			break;
		case 2:
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "bullchicken/bc_attackgrowl3.wav", 1.0f, ATTN_NORM);
			break;
		}
	...
	case static_cast<int>(SquidTask::TASK_SQUID_HOPTURN):
	{
		SetActivity(ACT_HOP);
...
MONSTERSTATE CBullsquid::GetIdealState(void)
{
	...
	{
		if (m_hEnemy != nullptr &&
			(iConditions & bits_COND_LIGHT_DAMAGE || iConditions & bits_COND_HEAVY_DAMAGE) &&
...
//=========================================================
// FollowerUse - Allow player to command friendly bullsquid
//=========================================================
void CBullsquid::FollowerUse(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	// Only players can use friendly bullsquids
	if (!pActivator || !pActivator->IsPlayer())
		return;

	// Only friendly bullsquids can be used
	if (!m_bFriendly)
		return;
	...
	if (!IsFollowing())
	{
		// If currently eating, interrupt it immediately
		if (m_pSchedule != nullptr)
		{
			int scheduleType = m_iScheduleIndex >= 0 && m_iScheduleIndex < m_pSchedule->cTasks ?
				static_cast<int>(SquidSchedule::EAT) : -1;
			// Check if current schedule is an eating schedule
			if (m_pSchedule == &slSquidEat[0] ||
				m_pSchedule == &slSquidSniffAndEat[0] ||
				m_pSchedule == &slSquidWallow[0])
			{
				// Interrupt eating and clear any pending tasks
				ClearSchedule();
			}
		}
...
//=========================================================
void CBullsquid::DeclineFollowing(void)
{
...
//=========================================================
void CBullsquid::StartFollowing(CBaseEntity* pLeader)
{
	if (pLeader == nullptr)
		return;
	...
	// Clear any eating schedule
	ClearSchedule();

	// Set the schedule to follow
	ChangeSchedule(GetScheduleOfType(static_cast<int>(SquidSchedule::FOLLOW)));
}

//=========================================================
// IsFollowing - Check if currently following a leader
//=========================================================
BOOL CBullsquid::IsFollowing(void)
{
...
//=========================================================
void CBullsquid::StopFollowing(BOOL bClearSchedule)
{
	// Clear the target
