...
void inline EMIT_SOUND_DYN1(edict_t* entity, int channel, const char* sample, float volume, float attenuation, int flags, int pitch)
{
	// Future maybe some captions or any ohter things
						   ...
						   int flags, int pitch)
{
	if (sample && *sample == '!')
	{
		...
		if (SENTENCEG_Lookup(sample, name) >= 0)
			EMIT_SOUND_DYN1(entity, channel, name, volume, attenuation, flags, pitch);
		else
			ALERT( at_aiconsole, "Unable to find %s in sentences.txt\n", sample );
	}
	else
		EMIT_SOUND_DYN1(entity, channel, sample, volume, attenuation, flags, pitch);
}
