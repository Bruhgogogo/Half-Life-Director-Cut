...
constexpr int COVER_CHECKS = 5;
constexpr int COVER_DELTA = 48;

constexpr float MONSTER_THINK_INTERVAL = 0.05; //How long before monster start next think
...
void CBaseMonster::Look(int iDistance)
{
    ...
    ClearConditions(bits_COND_SEE_HATE | bits_COND_SEE_DISLIKE | bits_COND_SEE_ENEMY |
        bits_COND_SEE_FEAR | bits_COND_SEE_NEMESIS | bits_COND_SEE_CLIENT);
...
{
    pev->nextthink = gpGlobals->time + MONSTER_THINK_INTERVAL; // keep monster thinking.
...
void CBaseMonster::Move(float flInterval)
{
    ...
    {
        flCheckDist = static_cast<float>(DIST_TO_CHECK);
    }
...
float CBaseMonster::ChangeYaw(int yawSpeed)
{
    ...
    if (current != ideal)
    {
        ...
        speed = static_cast<float>(yawSpeed) * delta * 2;
        move = ideal - current;
    ...
    for (i = 0; i < WorldGraph.m_cNodes; i++)
    {
        ...
        if (node.m_sHintType)
        {
            ...
            if (FValidateHintType(node.m_sHintType))
            {
                ...
                {
                    UTIL_TraceLine(pev->origin + pev->view_ofs, node.m_vecOrigin + pev->view_ofs,
...
BOOL CBaseMonster::FindLateralCover(const Vector& vecThreat, const Vector& vecViewOffset)
{
    ...
    for (i = 0; i < COVER_CHECKS; i++)
    {
        ...
        UTIL_TraceLine(vecThreat + vecViewOffset, vecLeftTest + pev->view_ofs,
            ignore_monsters, ignore_glass, ENT(pev) /*pentIgnore*/, &tr);
        ...
        // it's faster to check the SightEnt's visibility to the potential spot than to check the local move, so we do that first.
        UTIL_TraceLine(vecThreat + vecViewOffset, vecRightTest + pev->view_ofs,
            ignore_monsters, ignore_glass, ENT(pev) /*pentIgnore*/, &tr);
