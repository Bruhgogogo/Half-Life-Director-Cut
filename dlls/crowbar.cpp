...
constexpr float CROWBAR_PRIMARY_ATTACK_DURATION = 0.25f;
constexpr float CROWBAR_SECONDARY_ATTACK_DURATION = 0.8f;
constexpr float CROWBAR_PRIMARY_ATTACK_MISS_PENALTY = 0.5f;
constexpr float CROWBAR_SECONDARY_ATTACK_MISS_PENALTY = 1.0f;
...
int CCrowbar::Swing(int fFirst)
{
	...
	if (tr.flFraction >= 1.0f)
	{
		...
		{
			// miss
			m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + CROWBAR_PRIMARY_ATTACK_MISS_PENALTY;
	...
	m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + CROWBAR_PRIMARY_ATTACK_DURATION;
...
void CCrowbar::SecondaryAttack(void) // Why they add void to it? i don't understand anyways just follow it :(
{
	TraceResult tr;

	UTIL_MakeVectors(m_pPlayer->pev->v_angle);

	Vector vecFoward = gpGlobals->v_forward;

	Vector vecSrc = m_pPlayer->GetGunPosition();
	Vector vecEnd = vecSrc + vecFoward * 32.0f;
	...
	{
		// miss
		m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + CROWBAR_SECONDARY_ATTACK_MISS_PENALTY;

		SendWeaponAnim(CROWBAR_HEAVYATTACK);

		// player "shoot" animation
		m_pPlayer->SetAnimation(PLAYER_ATTACK1);

		EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/cbar_miss1.wav", 1.0f, ATTN_NORM);
	}
	else
	{
		SendWeaponAnim(CROWBAR_HEAVYATTACK);
		...
		if (pEntity)
		{
			...
			pEntity->TraceAttack(m_pPlayer->pev, gSkillData.plrDmgHeavyCrowbar, vecFoward, &tr, DMG_CLUB);
			...
			{
				// play thwack or smack sound
				EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/cbar_hhitbod.wav", 1.0f, ATTN_NORM);
	...
	m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + CROWBAR_SECONDARY_ATTACK_DURATION;
